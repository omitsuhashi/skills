# Loop Skill Operational Simplicity Spec

## 状態

Spec Gate 承認済み。Issue Gate、Execution Plan Gate、実装、push、PR 作成、remote write は未承認。

## Problem Statement

`grill-to-pr-loop` と `issue-implementation-loop` は、長い repository change を安全に進めるために、context contract、worker packet、resume brief、runtime state、review gate を備えた。ただし、現状は次の 3 つの運用摩擦が残っている。
...
この spec は、既存の user-facing skill 数と worker-only execution policy を維持したまま、loop 系の露出複雑性を下げる。
...
- planning prereq: `python3 skills/grill-to-pr-loop/scripts/check_prereqs.py --phase planning` passed。
- GitHub remote は存在するが `gh` auth は optional warning。remote write は未承認として扱う。
- `skill-architecture.toml` は repository-change-loop family の user-facing skill を `grill-to-pr-loop` と `issue-implementation-loop` の 2 つに固定している。
- `scripts/validate_skill_architecture.py --all` と `scripts/validate_skill_context.py --all` は current checkout で通る。
- `scripts/report_skill_context.py --all` は read-set ごとの estimated token / file count / headroom を返すが、gate 数、runtime artifact 数、worker-context requirement、human wait / review / delivery の有無を集約した operator-facing complexity を返さない。

## Accepted Decisions

- 新しい user-facing skill は追加しない。
- scheduler、runtime-state、review-gate、worker-contract、remote-delivery などの内部機構は standalone skill にしない。
- `grill-to-pr-loop` は planning / composition skill のままにし、implementation worker にはしない。
- `issue-implementation-loop` は approved packet から `PR_READY` までの execution coordinator のままにする。
- 小さい単発修正、単一ファイルの明確な修正、既存 issue 1 件の軽微な docs 修正には、原則として loop 系を使わない適用基準を明文化する。
- mental model は default read-set を大きくしない形で追加する。入口には短い要約を置き、詳細は operator-facing reference または durable synthesis として読む。
- workflow complexity metric は context metric と別軸にする。context budget が healthy でも、gate / artifact / worker / review / delivery の組み合わせで operational risk を警告できるようにする。
- remote write は未承認のため `local_only` とする。
...
1. **LSOS-001: loop 系の適用基準を明文化する**
   ...
   - 小さい単発修正は通常 skill / direct implementation に逃がす。

2. **LSOS-002: loop 系 mental model を 1 ページ化する**
...
3. **LSOS-003: workflow complexity report を追加する**
   - `scripts/report_skill_context.py --all --json` に、context metrics とは別の `workflow_complexity` summary を追加する。
   - 集計対象は gate 数、operation 数、runtime artifact / schema / template / script の数、worker-context requirement、review cycle、human wait、remote delivery policy などの operator-facing complexity とする。
...
4. **LSOS-004: regression tests と wiki ledger を更新する**
   - 適用基準、mental model の discoverability、workflow complexity JSON の shape を tests で固定する。
   - `knowledge/index.md` と `knowledge/log.md` に spec / issue ledger / implementation evidence を追加する。

## Acceptance Criteria

- `grill-to-pr-loop` は、使うべきケースと使わないケースを 1 screen 相当で説明している。
- `issue-implementation-loop` は、approved packet 前提、worker context 必須、coordinator 実装禁止の適用条件を入口で確認できる。
- loop 系の mental model は coordinator / worker / reviewer / runtime state / ledger / remote delivery の責務境界を 1 ページで説明している。
- mental model の追加により、`validate_skill_context.py --all` の budget validation が失敗しない。
...
- `validate_skill_architecture.py --all` は引き続き user-facing skill 2 件と forbidden standalone skill policy を検証する。
- 新しい standalone skill は追加されない。
- remote write は行わない。
...
GitHub issue / PR / push / merge は未承認のため実行しない。final PR merge は常に human-only。
...
- mental model を default read-set に入れた結果、context budget が悪化する。
...
- mental model を短くしすぎると、実行時に必要な stop condition まで省略される可能性がある。
