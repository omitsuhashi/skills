...
ただし、2026-07-01 の PR review feedback により、routine review で「将来的にやった方がいいかも」という future-only hardening を列挙させる運用は採用しない。Issue 実装レビューは Issue 意図適合、今回変更による regression、current PR delivery risk に限定する。

`classification_needed` と `hardening_candidate` 機構は完全には削除しない。ただし自動レビュー観点として探索しない。必要になった場合、または人間が明示した場合だけ扱う。credential、権限、データ破壊、security regression、production impact など delivery risk があるものは、勝手に直さず、かつ黙って通さず、`safety_escalation` として人間判断待ちにする。
...
- 承認済み Issue を越える future-only hardening、設計改善、保守性改善、追加検証、追加防御は routine review では列挙しない。
- `classification_needed` と `hardening_candidate` は自動レビュー観点として探さず、必要になった場合または人間が明示した場合だけ扱う。`hardening_candidate` は Issue completion / blocker release のために自動修正しない。
- final PR delivery 前に未判断の `hardening_candidate` が存在する場合だけ、人間へ提示し、取り込む / 後続へ送る / 採用しない / risk acceptance のどれかを明示決定する。
...
- `hardening_candidate` は「人間が明示的に依頼した hardening review、または current PR delivery risk に結び付くが承認済み Issue scope 外の提案」とする。future-only hardening は routine review では記録しない。
- `safety_escalation` は未承認 scope でも delivery risk が高いものとする。自動修正は禁止し、delivery 前に人間判断を必須にする。
- 分類不能な finding は `classification_needed` として、人間確認または coordinator の evidence-based classification を必要とする。`classification_needed` は自動レビュー観点として探さず、必要になった場合または人間が明示した場合だけ扱う。曖昧なまま自動修正しない。
...
| `hardening_candidate` | 明示依頼された hardening review、または current PR delivery risk に結び付く scope 外提案 | いいえ | 不可 | registry に残る場合だけ final PR 前に必要 |
...
- routine review は future-only hardening や classification-only pass を列挙しない。必要になった場合または明示依頼で記録する場合でも、`hardening_candidate` は 1 candidate 80 words 以下、1 issue 5 件以下を default とする。
- final PR 前の decision brief は、registry に未判断 candidate が存在する場合だけ作る。candidate ごとの full text ではなく、ID、source issue、summary、risk、recommended decision の compact table にする。詳細は `hardening-candidates.json` を path で参照する。
...
reviewer へ渡す packet は、現在の narrow review intent を維持する。自動レビュー観点は Issue 意図適合、今回変更による regression、current PR delivery risk の 3 つに限定する。classification-only pass と future-only hardening は通常レビュー観点から外す。
...
次に「今回変更による regression」と「current PR delivery risk」を確認する。
delivery risk は safety_escalation として返す。

将来的にやった方がよいだけの future-only hardening は列挙しない。
classification_needed と hardening_candidate は自動レビュー観点ではない。
必要になった場合、または人間が明示した場合だけ返す。
...
- `issue_id`
- `review_range`
- `intent_review.status`: `approved` / `changes_requested`
- `intent_findings[]`: `id`, `classification`, `severity`, `source_artifact`, `why_blocking`, `required_fix`
- `classification_needed[]`: 必要になった場合または明示された場合だけ返す。`id`, `source_issue`, `summary`, `missing_evidence`, `required_decision`
- `hardening_candidates[]`: 必要になった場合または明示された場合だけ返す。`candidate_id`, `source_issue`, `summary`, `rationale`, `suggested_change`, `risk_if_deferred`, `estimated_scope`, `delivery_blocker`
...
`hardening_candidate` は worker branch に混ぜず、runtime root の coordinator-owned artifact に集約する。ただし、routine review で future-only hardening を探して registry に入れる運用はしない。
...
- `safety_escalation` または実際に発生した `classification_needed` は、scope に応じて issue / descendants / resource / epic の `human_request_opened` を出す。
- 記録済み `hardening_candidate` は issue-level work を止めず、final delivery lane の pending decision として扱う。future-only hardening は通常レビューでは記録しない。
- resume brief は `Pending hardening decisions: N` と candidate registry path を短く出す。

### 4. Draft Final PR 後の Human Decision Gate

draft final PR を作成する前に、coordinator は `hardening-candidates.json` を読み、`pending_decision` がある場合だけ人間が読める compact table と PR body に集約する。
...
| --- | --- | --- | --- | --- | --- |
| automatic finding 分類 | Issue 実装レビュー中 | `superpowers:requesting-code-review` reviewer が 3 観点だけを確認し、coordinator が source artifact と照合して確定 | review packet、spec / issue path、acceptance criteria、review range | `intent_gap` / `implementation_regression` / `safety_escalation` | `intent_gap` / `implementation_regression` の scope 内 fix |
| 分類不能の扱い | 必要になった場合、または人間が明示した場合 | coordinator、必要なら人間 | reviewer finding、source artifact、write scope | issue-scoped `human_request_opened` または分類確定 | なし。曖昧なまま修正しない |
| candidate 登録 | 必要になった場合、または人間が明示した場合 | coordinator | 明示依頼、current PR delivery risk、または実際に発生した `classification_needed` / `safety_escalation` summary | `<runtime-root>/decisions/hardening-candidates.json` | registry への記録のみ |
...
- `skills/issue-implementation-loop/references/review-gate.md`
  - `superpowers:requesting-code-review` を第一候補とし、自動レビュー 3 観点、finding taxonomy、future-only hardening / classification-only pass 除外、candidate handling を追加する。
...
- 人間が明示的に依頼した hardening review を禁止すること。
- current PR delivery risk や `safety_escalation` を隠すこと。
...
1. **LRG-001: Review finding taxonomy と narrow review packet を定義する**
   - `review-gate.md` に自動レビュー 3 観点と future-only hardening / classification-only pass 除外を追加する。
...
- reviewer packet は、`intent_gap` / `implementation_regression` と `hardening_candidate` / `classification_needed` を混同せず、自動レビュー観点を Issue 意図適合、今回変更による regression、current PR delivery risk の 3 つに限定する wording を持つ。
...
- 明示依頼または current PR delivery risk として記録された `hardening_candidate` だけが final PR 前に人間判断 queue として提示される。
...
- **Implementation Review Gate**: 各 issue で自動レビュー 3 観点を必須にし、classification-only pass / future-only hardening を通常レビューから外し、記録済み hardening candidate を自動修正しないことを確認する。
...
- future-only hardening を routine review で列挙し、final PR 前の判断負荷を増やしている。
...
- 明示依頼または current PR delivery risk ではない `hardening_candidate` が増えると final PR 前の判断負荷が上がる。routine review では future-only hardening を列挙しない。
