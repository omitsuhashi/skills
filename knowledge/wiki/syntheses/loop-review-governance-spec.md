...
`hardening_candidate` 機構は完全には削除しない。人間が明示的に hardening review を依頼した場合、または current PR delivery risk に結び付く場合だけ記録できる。credential、権限、データ破壊、security regression、production impact など delivery risk があるものは、勝手に直さず、かつ黙って通さず、`safety_escalation` として人間判断待ちにする。
...
- 人間が明示的に hardening review を依頼した場合、または current PR delivery risk に結び付く場合だけ `hardening_candidate` として記録し、Issue completion / blocker release のために自動修正しない。
...
- routine review は future-only hardening を列挙しない。明示依頼または current PR delivery risk として記録する場合でも、`hardening_candidate` は 1 candidate 80 words 以下、1 issue 5 件以下を default とする。
...
reviewer へ渡す packet は、現在の narrow review intent を維持する。future-only hardening は通常レビュー観点から外す。
...
hardening_candidate は、人間が明示的に hardening review を依頼した場合、
または current PR delivery risk に結び付く場合だけ返す。
...
- `hardening_candidates[]`: 明示依頼または current PR delivery risk に結び付く場合だけ返す。`candidate_id`, `source_issue`, `summary`, `rationale`, `suggested_change`, `risk_if_deferred`, `estimated_scope`, `delivery_blocker`
...
| candidate 登録 | review intake 後、issue completion 前 | coordinator | 明示依頼または current PR delivery risk として返された `hardening_candidate` / `safety_escalation` summary | `<runtime-root>/decisions/hardening-candidates.json` | registry への記録のみ |
...
- `skills/issue-implementation-loop/references/review-gate.md`
  - `superpowers:requesting-code-review` を第一候補とし、finding taxonomy、future-only hardening 除外、candidate handling を追加する。
...
1. **LRG-001: Review finding taxonomy と narrow review packet を定義する**
   - `review-gate.md` に intent review と future-only hardening 除外を追加する。
...
- reviewer packet は、`intent_gap` / `implementation_regression` と `hardening_candidate` を混同せず、future-only hardening を通常レビュー観点から外す wording を持つ。
...
- **Implementation Review Gate**: 各 issue で intent review を必須にし、future-only hardening を通常レビューから外し、記録済み hardening candidate を自動修正しないことを確認する。
