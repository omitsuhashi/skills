...
### 4. Final PR 前の Human Decision Gate

final PR delivery plan を作る前に、coordinator は `hardening-candidates.json` を読み、`pending_decision` を人間へ提示する。
...
`pending_decision` が残ったまま final PR 作成、ready-for-review 化、merge へ進んではならない。
...
| final PR 前 candidate 採否 | delivery plan 作成前 | 人間 | compact decision brief、candidate registry path、coordinator 推奨 | `approved_for_current_pr` / `deferred_follow_up` / `declined` / `risk_accepted` | なし。採否決定は人間のみ |
...
| delivery preflight | final PR delivery plan validation 時 | coordinator / validator | runtime state、delivery plan、candidate registry | delivery allowed / blocked | 未判断候補がなければ draft final PR 作成 policy へ進む |

`safety_escalation` は final PR 前 candidate 採否ではなく delivery blocker として扱う。人間が risk acceptance するか、追加 issue として修正するまで delivery plan は通さない。

### 6. Delivery validation

`validate_delivery_plan.py` または delivery preflight は、final PR action の前に candidate registry を確認する。

- `hardening-candidates.json` が存在し、`pending_decision` または unresolved `safety_escalation` がある場合、final PR delivery plan は fail。
- `deferred_follow_up` / `declined` / `risk_accepted` は delivery を許可するが、completion report に residual risk として出す。
- `approved_for_current_pr` は、対応する `implementation_issue` が `PR_READY` / integrated / review approved でなければ delivery を許可しない。
...
3. **LRG-003: Final delivery preflight に pending candidate check を追加する**
   - final PR delivery plan validation が unresolved candidate を拒否する。
   - `approved_for_current_pr` は amendment issue 実装済みでなければ delivery 不可にする。
...
- `pending_decision` の候補が残ったまま final PR delivery plan が通らない。
...
- `pending_decision` の候補が残ったまま final PR delivery を進めようとしている。
