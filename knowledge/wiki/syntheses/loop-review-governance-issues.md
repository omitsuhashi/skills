...
Issue 実装レビューで、Issue 意図適合の blocking finding と、明示依頼または current PR delivery risk に限る candidate を混同しない review gate contract を追加する。future-only hardening は通常レビュー観点から外す。
...
- reviewer packet は Issue 意図適合レビューを必須 lane とし、future-only hardening を通常レビュー観点から外す。
