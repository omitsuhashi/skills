---
summary: Loop Review Governance Issue 台帳のローカルIssue、依存関係、実装結果、レビュー証跡と残課題を確認できる。
...
- Next phase entrypoint: draft final PR の ready-for-review は未承認。既存 hardening candidate 4 件は `deferred_follow_up` として current PR から除外済み。
...
- Open decisions: ready-for-review / merge は未承認。future-only hardening を通常レビュー観点から外す方針を反映済み。
- Pending risk: 既存 hardening candidate 4 件は `deferred_follow_up` として current PR の blocker ではない。`safety_escalation` が出た場合だけ ready-for-review / merge 前の人間判断が必要。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| loop-review-governance | LRG-001 | review finding taxonomy と narrow packet を定義する | 承認済み | final branch 統合済み | なし | LRG-002, LRG-004, LRG-005 | 未作成 | approved cycle 2 | [draft PR #27](https://github.com/omitsuhashi/skills/pull/27) |
...
## LRG-001: review finding taxonomy と narrow packet を定義する

### 目的

Issue 実装レビューで、自動レビュー観点を Issue 意図適合、今回変更による regression、current PR delivery risk の 3 つに限定し、明示依頼または必要時だけ扱う `classification_needed` / `hardening_candidate` と混同しない review gate contract を追加する。future-only hardening は通常レビュー観点から外す。
...
- reviewer packet は自動レビュー観点を Issue 意図適合、今回変更による regression、current PR delivery risk の 3 つに限定する。
...
- `classification_needed` は自動レビュー観点として探さず、必要になった場合または人間が明示した場合だけ扱う。
...
- 2026-07-01: `skills/issue-implementation-loop/references/review-gate.md` に finding taxonomy、`superpowers:requesting-code-review` 第一候補、`BASE_SHA..HEAD_SHA` committed range review、自動レビュー 3 観点、future-only hardening 除外、auto-fix 禁止、`classification_needed` 停止条件、default 600 words / hard 900 words の packet budget を追加した。
- `skills/issue-implementation-loop/tests/test_review_gate.py` を追加し、`intent_gap` と明示依頼 / current PR delivery risk に限る `hardening_candidate` の混同防止、paths-first review packet、future-only hardening 除外、fallback boundary を regression として固定した。
...
- `safety_escalation` / encountered `classification_needed` は必要最小 scope の `human_request_opened` へ送る。
...
- `runtime-state.md` に candidate registry path、candidate required fields、80 words / 5 件 default、`hardening_candidate` non-blocking、`safety_escalation` / encountered `classification_needed` の `human_request_opened` 境界を追加した。
- `human-wait.md` に `safety_escalation` / encountered `classification_needed` は smallest affected scope の `human_request_opened` にする契約を追加した。
...
| --- | --- | --- | --- | --- | --- |
| HC-LRG-002-001 | LRG-002 | `deferred_follow_up` | `deferred_follow_up` | LRG-002 review cycle 1。resume brief の `Pending hardening decisions: N` 動的表示。 | current PR には入れない |
| HC-LRG-003-001 | LRG-003 | `deferred_follow_up` | `deferred_follow_up` | runtime registry primary。`check_capabilities.py` / worker packet validation coverage。 | current PR には入れない |
| HC-LRG-003-002 | LRG-003 | `deferred_follow_up` | `deferred_follow_up` | runtime registry primary。`pending_hardening_candidates` / `residual_risks` の local-only completion report validation。 | current PR には入れない |
| HC-LRG-004-001 | LRG-004 | `deferred_follow_up` | `deferred_follow_up` | LRG-004 review cycle 1。acceptance criteria と `write_scope` の mismatch preflight。 | current PR には入れない |

既存 4 件は未判断ではない。current PR に追加実装せず、future-only hardening を通常レビュー観点から外す。LRG-005 は candidate decision surface の discoverability と regression evidence を同期する。
...
`loop-review-governance` の Issue Gate 承認済み local issue ledger。LRG-001 から LRG-005 の blocker graph、acceptance criteria、remote policy を定義する。
