...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| loop-review-governance | LRG-001 | review finding taxonomy と 2 lane packet を定義する | 承認済み | worker 実装済み | なし | LRG-002, LRG-004, LRG-005 | 未作成 | 未実施（worker commit 後に候補 range を提示） | 未作成 |
| loop-review-governance | LRG-002 | candidate registry と bounded decision artifact を追加する | 承認済み | worker 実装済み | LRG-001 | LRG-003, LRG-004, LRG-005 | 未作成 | 未実施（worker commit 後に候補 range を提示） | 未作成 |
| loop-review-governance | LRG-003 | final delivery preflight に pending candidate check を追加する | 承認済み | worker 実装済み | LRG-002 | LRG-005 | 未作成 | 未実施（worker commit 後に候補 range を提示） | 未作成 |
...
- 実行可能: なし。LRG-001 / LRG-002 / LRG-003 は worker 実装済みで、implementation review / blocker release 待ち。
- ブロック中: LRG-004、LRG-005。
- Cyclic blocker: なし。
...
- 2026-07-01: `validate_delivery_plan.py` が runtime-state path の sibling `<runtime-root>/decisions/hardening-candidates.json` を読み、final PR preflight で未判断 candidate を拒否するようにした。
- `pending_decision` と unresolved `safety_escalation` は final PR delivery plan を fail し、`approved_for_current_pr` は `implementation_issue` が `PR_READY` / integrated / review approved のいずれかでなければ fail する。
- `deferred_follow_up` / `declined` / `risk_accepted` は delivery を許可し、`--json` output の `residual_risks` に残す。未判断 candidate は `pending_hardening_candidates` に出す。
- `remote-delivery.md` に final PR preflight と `local_only` completion report の `pending_hardening_candidates` / residual risk contract を追加した。final PR merge、ready-for-review、force push、deploy、credential、permission、billing、production、destructive action の別承認境界は維持した。
