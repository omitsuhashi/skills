...
> **Status:** 2026-07-28にHumanが現状のlow / medium / high mappingを維持し、task complexity / riskをrole defaultより優先する方針を承認した。local implementation、independent review、knowledge closeoutを実行する。
...
- [ ] `superpowers:writing-skills`と`superpowers:test-driven-development`を適用する。
- [ ] high-risk task reviewがhighになる明示規則と、共有default vocabularyがlow / medium / highに限定されることをcontract testへ追加する。
- [ ] focused testを実行し、既存Skillで期待どおりREDになることを確認する。
- [ ] `SKILL.md`へtask complexity / riskがrole defaultより優先される規則を最小追加する。
- [ ] focused testを再実行しGREENを確認する。
- [ ] implementer reportとdiffを独立task reviewerへ渡し、material findingを解消する。
...
- [ ] canonical design、index、append-only logを実装結果へ同期する。
- [ ] focused tests、repository validators、dual-host compatibility、Skill quick validation、`git diff --check`をfreshに実行する。
