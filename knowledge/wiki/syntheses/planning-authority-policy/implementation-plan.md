...
- [ ] `scripts/test_validate_skill_architecture.py` で `validate_policy` を import し、default policy を深い copy にして直接 mutate できる helper を追加する。
- [ ] 次の5ケースを `SkillArchitecturePolicyTests` に追加する。
...
- [ ] `skill-architecture.toml` に次の table だけを追加する。
...
- [ ] model / reasoning の具体値、agent ID、routing rule は追加しない。

### 1.3 closed exact-value validator を実装する

- [ ] `scripts/validate_skill_architecture.py` に `EXPECTED_PLANNING_AUTHORITY_POLICY` を追加する。
- [ ] `_validate_planning_authority_policy(family, errors)` を追加し、次を順に検証する。
...
- [ ] `validate_policy` 末尾で、既存 `_validate_context_compaction_policy` と並べて新 helper を呼ぶ。
- [ ] small TOML parser、`schema_version = 1`、既存3 policy group の検証は変更しない。
...
- [ ] production files が PAP-001 write scope 内だけであることを確認する。
- [ ] scoped commit を作成する。
...
- [ ] `GrillToPrLoopTests` に `test_planning_authority_contract_separates_integration_advice_and_decision` を追加する。
- [ ] entrypoint と planning contract の連結 text に、次の意味を持つ exact token / sentence があることを assert する。
...
- [ ] entrypoint word budget `<= 950` の既存 test を維持する。
...
- [ ] `Required Rules` に次の3 owner を短く追加する。
...
- [ ] host runtime がmodel / reasoningを選び、repo artifactは具体値を保存しないことを1文で明示する。
- [ ] entrypoint へ dispatch手順や長い例を持ち込まない。

### 2.3 Planning reference に dispatch contract を追加する

- [ ] `planning-contract.md` に `## Planning Authority / Supporting Agent Dispatch` を追加する。
- [ ] supporting dispatch を次で限定する。
...
- [ ] grill tests、context validators、budget reportを実行する。
...
- [ ] report が hard limit を越える場合、entrypointではなく planning reference の重複を削る。authorityを別 skill / referenceへ分裂させない。
- [ ] scoped commit を作成する。
...
- [ ] new test file は `Path(__file__).resolve()` から repo root を求め、`scripts` を `sys.path` へ追加して repository small TOML parserを再利用する。TOML parserを複製しない。
- [ ] `PlanningAuthorityPolicyTests` に次の4 test を追加する。
...
- [ ] focused test を実行する。PAP-001 / PAP-002 が正しく統合済みなら初回から pass してよい。この Issue のTDD対象は「新しい回帰テスト自体が既存契約を観測できること」であり、production schemaへ故意の変更は加えない。
...
- [ ] test file の作業中だけ policy payloadのlocal copyへ `model_name` を追加し、assert helper が failすることを確認する。production fileは変更しない。
- [ ] negative-control を戻し、focused test を再実行して pass させる。
- [ ] `git diff -- skills/issue-implementation-loop/assets skills/issue-implementation-loop/scripts` が空であることを確認する。

### 3.3 Cross-loop verification と commit

- [ ] full grill / execution tests を実行する。
...
- [ ] `issues.md` の各 Issue に implementation commit、verification、review verdictを記録し、dependency release順と最終状態を同期する。
- [ ] `implementation-plan.md` の完了 checkbox と実際の結果が一致するよう更新する。未実行項目を完了扱いしない。
- [ ] `knowledge/index.md` の Planning Authority Policy entry から spec、Issue台帳、実装計画、sealed Input Packetを発見可能にする。
- [ ] `knowledge/log.md` に implementation / review closeout を append-only で追加する。
- [ ] spec bytes と sealed Input Packet bytes は変更しない。変更が必要なら実装を止め、該当Gateへ戻す。

### 4.2 Full verification を fresh に実行する

- [ ] 次の順で実行し、command / result / failure修正を closeout evidenceへ残す。
...
- [ ] default checkout `/Users/omitsuhashi/repos/omitsuhashi/skills` が planning開始時の `HEAD=f5d151e34de5089d75be68249e09da8a2d14f282` と開始時statusを維持していることをread-onlyで確認する。
- [ ] production diff が approved write scope 内で、execution schemas / validator / scheduler / runtimeに変更がないことを確認する。

### 4.3 Closeout commit と review

- [ ] scoped closeout commit を作成する。
...
- [ ] approved 5-field policyがmachine-readable正本とhuman-readable contractで一致する。
- [ ] supporting agents は advisory/read-onlyで、Human decision authorityが維持される。
- [ ] model / reasoning の具体値が新規 durable artifact fieldとして存在しない。
- [ ] execution schema / runtime implementationにproduction diffがない。
- [ ] worker-only / fresh-or-compacted coordinator contractが回帰していない。
- [ ] 全verificationと2種類のreviewが承認済みである。
- [ ] default checkoutが開始時snapshotから不変である。
- [ ] remote actionがゼロである。
