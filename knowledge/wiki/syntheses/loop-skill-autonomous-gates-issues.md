...
Spec Gate / Issue Gate 承認済み。Execution Plan Gate は agent preflight + commit boundary として自動通過済み。Execution Envelope prepare 済み。LSAG-001 から LSAG-004 は worker 実装 / coordinator review 承認済み。LSAG-005 の full regression base mismatch に対し、2026-06-30 の人間承認で LSAG-006 integration base work item を追加し、Execution Envelope revision 2 とした。GitHub issue mirror、push、PR 作成、merge はまだ行っていない。承認済み仕様は [Loop Skill 自動継続 Gate 仕様](loop-skill-autonomous-gates-spec.md)、normalized input packet は [Loop Skill 自動継続 Gate Input Packet](loop-skill-autonomous-gates-input-packet.json)、Execution Envelope は [Loop Skill 自動継続 Gate Execution Envelope](loop-skill-autonomous-gates-execution-envelope.json)。
...
| loop-skill-autonomous-gates | LSAG-006 | LSAG-002 / LSAG-003 / LSAG-004 integration base を作成する | 承認済み | 実行可能 | LSAG-002, LSAG-003, LSAG-004 | LSAG-005 | 未作成 | 未実施 | 未作成 |
| loop-skill-autonomous-gates | LSAG-005 | regression tests と wiki discoverability を追加する | 承認済み | ブロック中 | LSAG-006 | なし | 未作成 | 未実施 | 未作成 |
...
- LSAG-006: 実行可能。LSAG-002、LSAG-003、LSAG-004 の reviewed head を含む integration base を作る。
- LSAG-005: ブロック中。LSAG-006 の integration head から regression と wiki discoverability を集約する。
...
LSAG-002 / LSAG-003 / LSAG-004 integration base を作成する

### 作るもの

LSAG-002、LSAG-003、LSAG-004 の reviewed head をすべて ancestor として含む integration branch を作る。これは LSAG-005 の full regression / final ledger / discoverability work の base を作るための execution-only work item であり、新しい機能仕様や acceptance criteria は追加しない。

### 受け入れ条件

- [ ] integration branch が LSAG-002、LSAG-003、LSAG-004 の reviewed head をすべて ancestor として含む。
- [ ] conflict resolution は既存 issue 実装の意味を変えず、shared ledger/log の統合だけに限定されている。
- [ ] LSAG-005 は integration head から分岐できる。
- [ ] GitHub push、PR 作成、merge は行わない。

### 非目標

- LSAG-002、LSAG-003、LSAG-004 の実装内容を変更すること。
- GitHub issue mirror、push、PR 作成、merge の実行。
...
- `git merge-base --is-ancestor 9a7fee240b283b65906ad8ff17239905b4c20d8c HEAD`
- `git merge-base --is-ancestor be8f906d814ef2dff7c6ca5bec6cdb6517f37c07 HEAD`
- `git merge-base --is-ancestor 5a9e77ed608fd46f1e7901aad12686cf4eb83333 HEAD`
- `git diff --check`
