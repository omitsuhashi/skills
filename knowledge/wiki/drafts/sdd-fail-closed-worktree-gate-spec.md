---
title: SDD fail-closed worktree gate 仕様 proposal
...
review_state: awaiting-human-written-spec-review
confidence: high
...
evidence:
  ...
  - knowledge/wiki/syntheses/sdd-first-write-worktree-migration-spec.md
created_actor: maintainer-delegated SDD synthesis worker
direct_update_unavailable_reason: written spec not yet Human-approved
requested_owner_action: promote after approval
---

# SDD fail-closed worktree gate 仕様 proposal

> [!warning] Proposed draft
> 本書は maintainer-delegated actor が作成した未承認 proposal であり、verified
> canonical specification ではない。Human が Written Spec を承認した後にだけ
> `knowledge/wiki/syntheses/sdd-fail-closed-worktree-gate-spec.md` へ promote する。
> 承認前は `knowledge/index.md` と `knowledge/log.md` を更新しない。

## Draft status

- current status: `proposed`
- review state: `awaiting-human-written-spec-review`
...
- direct canonical update unavailable reason: Written Spec がまだ Human-approved
  ではない
...
binding と unrelated Epic adapter を引き続き所有する。本 proposal が Human
approval 後に canonical になった場合、repository-change first-entry precedence、
relevant failure の no-fallback semantics、guard preflight / activation / commit
behavior については本 addendum を優先する。承認前の draft は既存 canonical
spec を override しない。

Human approval 後の llm-wiki promotion write set と implementation write set は
混在させない。
...
1. Human が本 Written Spec を承認するまで draft のまま保持する。
2. Human approval 後、implementation に先立つ独立 llm-wiki promotion task で、
   draft lifecycle、canonical synthesis、`knowledge/index.md`、
   `knowledge/log.md` を一つの promotion sync set として同期する。
...
から除外した。draft-note 必須 fields、canonical target identity、proposal/evidence、
authority、requested owner action を保持し、未承認 draft を active canonical
knowledge として扱っていない。
...
Human Written Spec approval が次の gate である。
