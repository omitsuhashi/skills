...
- [SDD Compatibility Removal Follow-up Plan](wiki/syntheses/sdd-compatibility-removal-follow-up-plan.md) — agent-agnostic behaviorを維持し、非要件となったcross-runtime compatibility policy、validator、CI gateを削除してlocal closeoutしたcurrent follow-up plan。
  検索語: sdd-implementation, compatibility removal, cross-runtime, validator removal, CI gate removal, plugin target, historical dual-host, local closeout, PR-ready, follow-up plan, 互換性削除, 実装計画
