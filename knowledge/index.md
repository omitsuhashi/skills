...
- [Task Management Write / Preflight Execution Envelope](wiki/syntheses/portfolio-os-task-backend-plugin-skill-write-preflight-execution-envelope.json) — worker-only実行、issue branch/worktree予約、single-base integration ownership、最大2 review cycleを固定したschema v3 / revision 3 Execution Envelope。POTASK-013 regression guard scope amendmentを含む。
