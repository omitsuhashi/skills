...
- [Task Management Write / Preflight Interface 仕様](wiki/syntheses/task-management-write-preflight-interface-spec.md) — task-management public interface v2、unified route v2、approval digest、confidence-aware human confirmation、preflight / apply、typed result normalizationを定義するSpec Gate承認済み仕様。
  検索語: task-management, task_preflight, task_apply, task-management-write, approval digest, approval receipt, confidence_authorized, human confirmation, route v2, TaskPreflightResult, TaskWriteResult, Companies contract, fail closed
- [GitHub Projects Task Backend Adapter 仕様](wiki/syntheses/task-adapter-github-projects-spec.md) — separate GitHub Projects Adapter plugin、official GitHub MCP tool mapping、linked Issue create、field update、read-back、partial failure、provider data遮断を定義するSpec Gate承認済み仕様。
...
- [Portfolio OS Task Backend Plugin Skill Issues](wiki/syntheses/portfolio-os-task-backend-plugin-skill-issues.md) — `portfolio-os-task-backend-plugin-skill` のcanonical local-first ledger。POTASK-012、014、016、017、018のlocal PR_READY evidence、POTASK-013 human wait、POTASK-015 / 019 blockerを集約する。
  検索語: Portfolio OS, task backend, task-management, local issue, Issue Gate, POTASK-012, POTASK-013, POTASK-014, POTASK-016, POTASK-017, POTASK-018, POTASK-019, WAITING_HUMAN, approval binding, GitHub Projects Adapter, blocker graph, task_apply
