...
- [[wiki/syntheses/llm-wiki-authoring-discovery-diagnostics-implementation-plan|llm-wiki Authoring Discovery Diagnostics 実装計画]] — 2026-08-08にHumanが明示承認したcurrent implementation plan。no-guidance / current-control REDとchanged-skill GREENのfresh-context pressure scenario、semantic selector、5つのdiscovery diagnostic condition、current-state precedence、docs-only write set、task review、durable closeout、whole-branch reviewを実行順に固定する。production implementationは未実施。
  検索語: llm-wiki, authoring discovery diagnostics, semantic selector, evidence-bearing BLOCKED, discovery unavailable, unobserved candidates, current contract precedence, pressure scenario, RED GREEN, no guidance control, skill present, Human approved, current implementation plan, Plan Gate approved, docs only, 実装計画, 計画承認, 再発防止
