...
- [Portfolio OS Task Backend Plugin Skill Spec](wiki/syntheses/portfolio-os-task-backend-plugin-skill-spec.md) — Portfolio OS task state-free方針を保つ全体仕様とPOTASK-019 local integration evidence。task-management v0.4.0、separate GitHub Projects Adapter v0.1.0、repo-only completionと別Live Activation Gateを追跡し、public Interface本文は一意なnormative canonicalへ委譲する。
  検索語: Portfolio OS, task backend, task-management, plugin package, primary skill, GitHub Projects Adapter, v0.4.0, v0.1.0, Interface v2, POTASK-019, local integration, live activation gate, no gh, Hermes Agent
- [Task Management Write / Preflight Interface 仕様](wiki/syntheses/task-management-write-preflight-interface-spec.md) — Companiesへ渡すpublic Interface v2の唯一のnormative reference。`task_query` / `task_preflight` / `task_apply`、unified route v2、approval digest、confidence-aware human confirmation、typed result normalizationを定義し、local implementation / fake verification完了と別Live Activation Gateを記録する。
  検索語: task-management, task_query, task_preflight, task_apply, task-management-write, approval digest, approval receipt, confidence_authorized, human confirmation, route v2, TaskPreflightResult, TaskWriteResult, Companies contract, unique canonical, fail closed
- [GitHub Projects Task Backend Adapter 仕様](wiki/syntheses/task-adapter-github-projects-spec.md) — separate GitHub Projects Adapter plugin、official GitHub MCP tool mapping、linked Issue create、field update、read-back、partial failure、provider data遮断を定義し、v0.1.0 local implementation / fake verification完了と別Live Activation Gateを記録する仕様。
  検索語: task-adapter-github-projects, GitHub Projects, GitHub MCP Server, projects_write, issue_write, add_issue_comment, linked issue, preflight, partial failure, adapter contract v2, no draft item
- [Task Management Write / Preflight 実装計画](wiki/syntheses/2026-07-21-task-management-write-preflight-implementation-plan.md) — 承認済みSpecをIssue Gate後にtask-management write facadeとseparate GitHub adapterへTDD実装するfile-level計画とverification matrix。
...
- [Portfolio OS Task Backend Plugin Skill Issues](wiki/syntheses/portfolio-os-task-backend-plugin-skill-issues.md) — `portfolio-os-task-backend-plugin-skill` のcanonical local-first ledger。POTASK-012から019のlocal PR_READY evidence、POTASK-019のexact integration / TDD / full verification / independent cycle-2 review、Epic Base local統合、remote / live未実施境界を集約する。
  検索語: Portfolio OS, task backend, task-management, local issue, Issue Gate, POTASK-012, POTASK-019, PR_READY, approval binding, GitHub Projects Adapter, Interface v2, implementation evidence, cycle 2 review, epic base, live activation gate, task_apply
...
- [Task Management Write / Preflight Execution Envelope](wiki/syntheses/portfolio-os-task-backend-plugin-skill-write-preflight-execution-envelope.json) — worker-only実行、issue branch/worktree予約、single-base integration ownership、最大2 review cycleを固定したschema v3 / revision 5 Execution Envelope。POTASK-013 regression guard、POTASK-019 durable completion evidence、human-approved normative status/current-boundary scope amendmentを含む。
