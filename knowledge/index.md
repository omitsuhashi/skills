...
- [[wiki/syntheses/keep-implementation-simple-plan|Keep Implementation Simple V2 実装計画]] — `KIS-PLAN-2026-08-14-V2`。KISを変更せず、既存SDD surfaceと一つのinline-data regressionだけで同じcanonical pathを正確な7 roleへ渡す2-task plan。independent re-reviewは`ready`で、Control Returnは`complete`、local Implementation Stage entryは`allowed`である。
  検索語: keep-implementation-simple, KIS-PLAN-2026-08-14-V2, KIS-SPEC-2026-08-14-V2, sdd-implementation, seven roles, canonical path, read fully before work, commit gate, wiki validation, independent plan re-review ready, Implementation Stage allowed, 実装計画
