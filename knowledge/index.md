...
- [[wiki/syntheses/llm-wiki-authoring-responsibility-separation-spec|llm-wiki authoring 責務分離仕様]] — 承認・実装済みの責務分離を維持しつつ、Authoring Profileのsemantic selector、evidence-bearing `BLOCKED`、current-state precedenceを追加するfocused revisionのcurrent Written Spec。reviewed two-file docs-only contractはlandedし、implementation closeout candidateとしてfinal whole-branch reviewを待つ。
  検索語: llm-wiki, authoring responsibility separation, authoring discovery, semantic selector, Authoring Profile, Compatibility Requirement, evidence-bearing BLOCKED, discovery unavailable, unobserved candidates, current-state precedence, historical baseline, implementation closeout candidate, final whole-branch review pending, docs only, 責務分離, discovery診断, 再発防止, closeout
...
- [[wiki/syntheses/llm-wiki-authoring-discovery-diagnostics-implementation-plan|llm-wiki Authoring Discovery Diagnostics 実装計画]] — 2026-08-08のHuman承認計画を、reviewed two-file docs-only production contractとRED-to-GREEN pressure-gate completionを記録したImplementation closeout candidateへ同期した。final whole-branch reviewはpendingであり、approved task contractは維持する。
  検索語: llm-wiki, authoring discovery diagnostics, semantic selector, evidence-bearing BLOCKED, discovery unavailable, unobserved candidates, current-state precedence, pressure scenario, RED GREEN, implementation closeout candidate, final whole-branch review pending, reviewed Task 1, docs only, 実装計画, closeout, 再発防止
