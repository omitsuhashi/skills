...
- [Loop Skill Approved Spec Binding Contract 仕様](wiki/syntheses/loop-skill-approved-spec-binding-contract-spec.md) — `grill-to-pr-loop` が人間承認済み spec を exact raw-byte digest へ seal し、`issue-implementation-loop` が prepare から delivery まで同じ packet-rooted binding を fail closed に再検証する local 実装・検証済み clean-break 契約。
...
- [Loop Skill Approved Spec Binding Contract Issues](wiki/syntheses/loop-skill-approved-spec-binding-contract-issues.md) — ASBC-001〜ASBC-006 の task-local reviewに加え、初回 closeoutを再オープンした2段階の whole-branch hardening、metadata parity micro-fix、final zero-finding approval、245/25/6 tests、残余リスク、remote未実行境界を集約する全件 `LOCAL_COMPLETE` の最終 local-first ledger。
  検索語: ASBC, approved spec binding, LOCAL_COMPLETE, re-closeout, whole-branch review, hardening review, metadata parity, zero findings, commit range, acceptance matrix, forward test, 245 tests, residual risks, local_only
- [Loop Skill Approved Spec Binding Contract 実装計画](wiki/syntheses/2026-07-21-loop-skill-approved-spec-binding-contract-implementation-plan.md) — 6 task のRED/GREEN実装後、2 Critical/6 Importantと2 Critical/1 Important/1 Minorを閉じたwhole-branch hardening addendumまで完了した Execution Plan artifact。
  検索語: ASBC, completed implementation plan, approved spec binding, hardening addendum, Execution Plan Gate, TDD, current-only, final review, dual-host, local_only
- [Loop Skill Approved Spec Binding Contract Input Packet](wiki/syntheses/loop-skill-approved-spec-binding-contract-input-packet.json) — current executable Input Packet v2。exact spec binding、6項目の approval evidence、ASBC-001〜ASBC-006 の execution intent、`local_only` delivery intent の唯一の正本。
  検索語: ASBC, current Input Packet v2, approved spec binding, spec digest, approval evidence, work items, local_only, executable artifact, seal
- [Loop Skill Approved Spec Binding Contract Execution Envelope](wiki/syntheses/loop-skill-approved-spec-binding-contract-execution-envelope.json) — current executable corrected Execution Envelope v4。sealed packetの全work item・scope・dependency・local-only intentをexact projectionし、full gate commit、Epic-base blobs、worker-only execution、review cycleを固定する。
  検索語: ASBC, corrected Execution Envelope v4, exact intent projection, approved_spec_binding, gate commit, epic base blobs, dependency edges, packet digest, worker-only, local_only
