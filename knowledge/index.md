...
- [SDD Implementation Skill 設計](wiki/syntheses/sdd-implementation-skill-design.md) — Superpowersを開発方法論の正本とし、spec stageへGrill with Docs、durable knowledge lifecycleへLLM Wiki、host boundaryへmodel resolutionとoptional reasoning effortを組み込むHuman-approved Written Spec。
...
- [SDD Implementation Superpowers-first Revision 実装計画](wiki/syntheses/sdd-implementation-superpowers-first-implementation-plan.md) — 承認済みWritten Specを、Skill lifecycle contract、repository routing、LLM Wiki closeoutの3 taskでTDD実装・検証するHuman-approved Execution Plan。
