...
- [[wiki/syntheses/keep-implementation-simple-spec|Keep Implementation Simple 仕様]] — minimum coherent current changeと削除testを正本にし、test・fixture・guardrailをcurrent evidenceへ限定するHuman承認済み仕様。`sdd-implementation`をsole/default routeに保ち、7種類のfresh workerへcanonical supporting Skillのread-before-workを要求する。
  検索語: keep-implementation-simple, sdd-implementation, minimum coherent change, deletion test, test contract, fixture contract, guardrail contract, required validation, fresh worker, supporting skill, 実装単純化, 最小十分, 過剰テスト, 仕様
- [[wiki/syntheses/keep-implementation-simple-plan|Keep Implementation Simple 実装計画]] — `KIS-SPEC-2026-08-14-V1`にbindingしたreviewed implementation plan。supporting Skillのsingle-source policyとSDDの7 role read-before-workを最小変更で実装し、fresh independent re-review `ready`によりlocal Implementation Stage entryがallowedである。
  検索語: keep-implementation-simple, KIS-SPEC-2026-08-14-V1, implementation plan, plan readiness, independent review, sdd-implementation, seven roles, read-before-work, single-source policy, fail closed, 実装計画, 計画準備完了
