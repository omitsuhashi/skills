...
- [Loop Skill Approved Spec Binding Contract Issues](wiki/syntheses/loop-skill-approved-spec-binding-contract-issues.md) — ASBC-001〜ASBC-006 の task-local reviewに加え、初回 closeoutを再オープンした2段階の whole-branch hardening、final zero-finding closure、244/25/6 tests、残余リスク、remote未実行境界を集約する全件 `LOCAL_COMPLETE` の最終 local-first ledger。
  検索語: ASBC, approved spec binding, LOCAL_COMPLETE, re-closeout, whole-branch review, hardening review, zero findings, commit range, acceptance matrix, forward test, 244 tests, residual risks, local_only
