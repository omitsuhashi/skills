...
- [[wiki/syntheses/task-management-hermes-readiness/spec|Task Management Hermes Schedule Secretary Readiness 仕様]] — operation-scoped preflight、Status-filtered pagination、reopen、portable/managed-native/live readinessの責務分離と外部deliveryを定義するpersonal-use MVPのWritten Spec Gate承認済みcurrent仕様。
  検索語: task-management, Hermes Agent, Schedule Secretary, GitHub MCP, operation-scoped preflight, reopen, Backlog, pagination, truncation, completeness, duplicate discovery, managed-native, readiness report, dry-run, apply, status, readback, 権限, セットアップ, タスク管理
- [[wiki/syntheses/task-management-hermes-readiness/implementation-plan|Task Management Hermes Schedule Secretary Readiness Implementation Plan]] — `Human-approved / current`。portable Skills Tasks 1〜5はexact reviewed head `50da5a9ab7e888d11e634a1fde88ddb3602a9730`でlocal implementation/review complete。`origin/main`競合解消後のDraft PR #47はopen / mergeable / CI成功で、Task 6 cross-repository handoffはPRの`origin/main` merge待ちでblocked。Companies current-main reconciliationと後続live Gateは未実行のまま保持する。
  検索語: task-management, Hermes Agent, Schedule Secretary, implementation plan, TDD, execution plan, Companies handoff, live authorization gate, 実装計画, テスト駆動
- [[wiki/syntheses/task-management-hermes-readiness/issues|Task Management Hermes Schedule Secretary Readiness Issue 台帳]] — `local-portable-review-complete`。Tasks 1〜5のexact commit、独立task review、portable acceptance mapping、5回のwhole-branch FAILとfix wave、最終PASS（Critical 0 / Important 0 / Minor 0）、Draft PR #47の公開状態を保持する台帳。Task 6はmerged revision待ちでblockedであり、overall/live readinessは未判定。
  検索語: task-management, Hermes Agent, Schedule Secretary, Issue ledger, review finding, fix wave, checkpoint hardening, Task 6 blocked, ローカルIssue, 修正履歴
...
- [[wiki/syntheses/sdd-implementation-skill-design.md|SDD Implementation Skill 設計]] — Superpowersを開発方法論の正本とし、Planning Controller、Control Return、fresh pre-implementation workerを含むskill behaviorを、入力・依存skill・active runtime capabilityだけで解決するcompatibility-freeのHuman-approved Written Spec。
  検索語: sdd-implementation, Superpowers, brainstorming, writing-plans, Subagent-Driven Development, SDD, Planning Controller, Control Return, fresh pre-implementation worker, Grill with Docs, active runtime, capability boundary, agent-agnostic, portable skill, compatibility-free, cross-runtime compatibility removal, reasoning effort, llm-wiki, knowledge lifecycle, Written Spec, 仕様作成, 仕様精緻化
- [[wiki/syntheses/sdd-preimplementation-context-isolation-spec.md|SDD 実装前コンテキスト分離仕様]] — Planning ControllerをHuman対話・判断・approval・routingへ限定し、repository調査、Spec Synthesis、Plan Authoringをfresh workerへ委譲するHuman-approved Written Spec。
  検索語: sdd-implementation, Planning Controller, Research Worker, Spec Synthesis Worker, Plan Author Worker, fresh context, Control Return, Stage Capsule, Decision Record, context isolation, duplicate question, 実装前調査, コンテキスト分離, 重複質問, 仕様作成, 実装計画
