...
- [SDD Compatibility Removal Follow-up Plan](wiki/syntheses/sdd-compatibility-removal-follow-up-plan.md) — agent-agnostic behaviorを維持し、非要件となったcross-runtime compatibility policy、validator、CI gateを削除したfinal-review-fix candidate。scoped re-reviewまではlocal completion / PR-readyではない。
  検索語: sdd-implementation, compatibility removal, cross-runtime, validator removal, CI gate removal, plugin target, historical dual-host, final review fix, scoped re-review pending, follow-up plan, 互換性削除, 実装計画
