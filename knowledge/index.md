...
- [[wiki/syntheses/sdd-portable-validation-simplification|SDD portable validation と責務単純化仕様]] — Human-approved amended active Written Spec。current amendmentは`LOCAL_COMPLETE`であり、copied root fixture / parity runnerを削除してcanonical direct validation、single semantic parser、runtime/test evidence splitをexisting ownerへ収束した。review head `81717fd`のCritical 0 / Important 3 / Minor 0は`791c2c8`とHuman-authorized `87fc8d5`でaddressされ、final scoped reviewはresidual `ADDRESSED`、new Critical 0 / Important 0 / Minor 0である。歴史的SPV `LOCAL_COMPLETE`はhistorical evidenceとして保持する。
  検索語: sdd-implementation, portable skill, direct Git probe, write-tree, BASELINE..HEAD, transient artifact, .superpowers, strict-zero post-policy, First-Write Worktree Gate, fail-closed containment, keep-implementation-simple, seven roles, staged deletion, ignored scratch, package closure, Superpowers ownership, sequential fallback, Written Spec, origin/main incorporation, 責務単純化, 検証
- [[wiki/syntheses/sdd-portable-validation-simplification-implementation-plan|SDD portable validation と責務単純化 実装計画]] — current fixture-minimization amendmentは`LOCAL_COMPLETE`。merge `56ab956`はcurrent mainをsecond parentとして両lineageとfeature treeを保持し、`1c38acd` / `254ab51`がdeleted fixture/runner、canonical direct validation、runtime/test splitを既存ownerで実装した。canonical review `81717fd`、bounded fix `791c2c8`、Human-authorized URL correction `87fc8d5`後のfinal scoped reviewはcleanでmaterial riskはない。GitHub publication / PR作成は別途Human-authorizedだが未実施で、local completion evidence外のcontroller-owned next actionである。
  検索語: sdd-implementation, portable validation, LOCAL_COMPLETE, historical LOCAL_COMPLETE, AM-1, AM-2, canonical whole-branch review clean, direct Git gate, isolated install, package closure, synthetic Git repository, canonical plan direct validation, strict zero, post policy history, First-Write Worktree Gate, exact seven roles, keep implementation simple, runtime test split, reviewed fix, URL correction, remote publication authorized, serialized integration, 実装計画, 責務単純化, 検証, closeout
...
- [[wiki/syntheses/keep-implementation-simple-spec|Keep Implementation Simple 仕様]] — `KIS-SPEC-2026-08-14-V2`。no-guidanceとunchanged Skillがともに5/5 GREENでRED未成立だったためKIS本文を変更せず、`sdd-implementation`だけで正確な7 fresh roleへ同じcanonical Skill pathと全文read-before-workを渡すHuman承認済み仕様。
  検索語: keep-implementation-simple, sdd-implementation, KIS-SPEC-2026-08-14-V2, unchanged skill, 5/5 GREEN, seven roles, canonical path, read fully before work, fail closed, sole default route, 仕様
- [[wiki/syntheses/keep-implementation-simple-plan|Keep Implementation Simple V2 実装計画]] — `KIS-PLAN-2026-08-14-V2`。approved six-file SDD wiring/test changeはtask review済みで、KIS-2のcombined verificationとdurable closeoutを完了したimplementation-closeout candidate。closeout commit/final-tree validationとFinal Reviewはpendingであり、`LOCAL_COMPLETE`は未宣言である。
  検索語: keep-implementation-simple, KIS-PLAN-2026-08-14-V2, KIS-SPEC-2026-08-14-V2, sdd-implementation, seven roles, canonical path, read fully before work, task review addressed, combined verification, wiki validation, implementation-closeout-candidate, final-review-pending, LOCAL_COMPLETE pending, 実装計画
