...
- [Loop Skill Approved Spec Binding Contract Issues](wiki/syntheses/loop-skill-approved-spec-binding-contract-issues.md) — ASBC-001〜ASBC-006 の commit range、review、ASB-01〜ASB-30、forward test、full verification、残余リスク、remote 未実行境界を集約する全件 `LOCAL_COMPLETE` の最終 local-first ledger。
  検索語: ASBC, approved spec binding, LOCAL_COMPLETE, final ledger, commit range, review evidence, acceptance matrix, forward test, full verification, residual risks, local_only, grill-to-pr-loop, issue-implementation-loop
- [Loop Skill Approved Spec Binding Contract 実装計画](wiki/syntheses/2026-07-21-loop-skill-approved-spec-binding-contract-implementation-plan.md) — core bootstrap seal、current-only artifact chain、state-changing guards、skill contract、fresh-agent forward test、full verification を 6 task の RED/GREEN/scoped commit で完了した Execution Plan artifact。
  検索語: ASBC, completed implementation plan, approved spec binding, Execution Plan Gate, TDD, current-only, bootstrap seal, forward test, dual-host, local_only
...
- [Loop Skill Approved Spec Binding Contract Execution Envelope](wiki/syntheses/loop-skill-approved-spec-binding-contract-execution-envelope.json) — current executable Execution Envelope v4。sealed packet raw digest、full gate commit、worker-only execution、review cycle、local-only remote policy を固定する。
  検索語: ASBC, current Execution Envelope v4, approved_spec_binding, gate commit, packet digest, worker-only, review cycle, local_only, executable artifact
