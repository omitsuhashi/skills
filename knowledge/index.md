...
- [[wiki/syntheses/llm-wiki-authoring-responsibility-separation-spec|llm-wiki authoring 責務分離仕様]] — 承認・実装済みの責務分離を維持しつつ、Authoring Profileのsemantic selector、evidence-bearing `BLOCKED`、current-contract precedenceを追加するfocused revisionのHuman承認済みcurrent Written Spec。Plan Stageとproduction contract変更は別承認まで未実施。
  検索語: llm-wiki, authoring responsibility separation, authoring discovery, semantic selector, Authoring Profile, Compatibility Requirement, evidence-bearing BLOCKED, discovery unavailable, unobserved candidates, current contract precedence, historical baseline, obsidian, obsidian-markdown, Human approved, current Written Spec, 責務分離, discovery診断, 再発防止, 仕様承認
...
- [[wiki/syntheses/llm-wiki-authoring-discovery-diagnostics-implementation-plan|llm-wiki Authoring Discovery Diagnostics 実装計画]] — Human承認済みfocused Written Specにbindingしたrevised implementation-plan review candidate。no-guidance / current-control REDとchanged-skill GREENのfresh-context pressure scenario、semantic selector、5つのdiscovery diagnostic condition、current-state precedence、docs-only write set、task review、durable closeout、whole-branch reviewを実行順に固定する。HumanのPlan Gate承認前はnon-executable。
  検索語: llm-wiki, authoring discovery diagnostics, semantic selector, evidence-bearing BLOCKED, discovery unavailable, unobserved candidates, current contract precedence, pressure scenario, RED GREEN, no guidance control, skill present, implementation plan candidate, Plan Gate pending, docs only, 実装計画, 計画レビュー, 再発防止
