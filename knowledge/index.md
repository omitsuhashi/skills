...
- [[wiki/syntheses/task-management-hermes-readiness/issues|Task Management Hermes Schedule Secretary Readiness Issue 台帳]] — Tasks 1〜5のexact commit、独立task review、portable acceptance mapping、whole-branch FAILとfix waveを記録するlocal-first台帳。修正後reviewはpending、Task 6はmerged revision待ちでblocked。
  検索語: task-management, Hermes Agent, Schedule Secretary, implementation plan, TDD, operation capability matrix, canonical identity, lossless pagination, reopen, metadata preservation, Companies handoff, live authorization gate, 実装計画, テスト駆動
