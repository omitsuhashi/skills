...
- [[wiki/syntheses/task-management-hermes-readiness/implementation-plan|Task Management Hermes Schedule Secretary Readiness Implementation Plan]] — `Human-approved / current`。portable Skills Tasks 1〜5とinitial whole-branch findingのfix waveはlocal実装済みで、修正後whole-branch reviewはpending。Task 6 cross-repository handoffはmerged revision待ちでblocked、Companies current-main reconciliationと後続live Gateは未実行のまま保持する。
...
- [[wiki/syntheses/task-management-hermes-readiness/issues|Task Management Hermes Schedule Secretary Readiness Issue 台帳]] — Tasks 1〜5のexact commit、独立task review、portable acceptance mapping、whole-branch FAILとfix waveを記録するlocal-first台帳。修正後reviewはpending、Task 6はmerged revision待ちでblocked。
