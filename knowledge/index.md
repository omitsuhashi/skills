...
- [Portfolio OS Task Backend Plugin Skill Issues](wiki/syntheses/portfolio-os-task-backend-plugin-skill-issues.md) — `portfolio-os-task-backend-plugin-skill` のcanonical local-first ledger。POTASK-012から018のlocal PR_READY evidenceと、POTASK-019のexact integration、TDD、full verification、独立review待ち、remote / live未実施を集約する。
  検索語: Portfolio OS, task backend, task-management, local issue, Issue Gate, POTASK-012, POTASK-019, REVIEWABLE, approval binding, GitHub Projects Adapter, Interface v2, implementation evidence, live activation gate, task_apply
...
- [Task Management Write / Preflight Execution Envelope](wiki/syntheses/portfolio-os-task-backend-plugin-skill-write-preflight-execution-envelope.json) — worker-only実行、issue branch/worktree予約、single-base integration ownership、最大2 review cycleを固定したschema v3 / revision 5 Execution Envelope。POTASK-013 regression guard、POTASK-019 durable completion evidence、normative status/current-boundary scope amendmentsを含む。
