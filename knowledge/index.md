...
- [[wiki/syntheses/sdd-portable-validation-simplification|SDD portable validation と責務単純化仕様]] — Human-approved amended active Written Spec。current amendmentはreviewed AM-1 implementationとfresh combined verificationを完了した`implemented-pending-final-review`であり、copied root fixture / parity runnerを削除してcanonical direct validation、single semantic parser、runtime/test evidence splitをexisting ownerへ収束した。歴史的SPV `LOCAL_COMPLETE`は保持し、current amendmentのfinal status根拠にはしない。
...
- [[wiki/syntheses/sdd-portable-validation-simplification-implementation-plan|SDD portable validation と責務単純化 実装計画]] — historical SPV `LOCAL_COMPLETE` evidenceを保持しつつ、current fixture-minimization amendmentはAM-1 reviewed completionとAM-2 phase-one knowledge/verification synchronizationを完了した`implemented-pending-final-review`である。merge `56ab956`はcurrent mainをsecond parentとして両lineageとfeature treeを保持し、`1c38acd` / `254ab51`がcanonical direct validation、deleted fixture/runner、runtime/test splitを実装した。exactly one canonical whole-branch reviewとdependent final status transitionはpendingであり、remote writeは未実施である。
  検索語: sdd-implementation, portable validation, implemented-pending-final-review, historical LOCAL_COMPLETE, AM-1, AM-2, canonical whole-branch review pending, direct Git gate, isolated install, package closure, synthetic Git repository, canonical plan direct validation, strict zero, post policy history, First-Write Worktree Gate, exact seven roles, keep implementation simple, runtime test split, serialized integration, 実装計画, 責務単純化, 検証, closeout
