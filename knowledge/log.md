...
## [2026-07-01] decision | Loop review governance Execution Plan Gate approved

- Execution Plan Gate 承認済み artifact として `knowledge/wiki/syntheses/loop-review-governance-input-packet.json`、`knowledge/wiki/syntheses/loop-review-governance-execution-envelope.json`、`knowledge/wiki/syntheses/loop-review-governance-handoff-brief.md` を扱う
- Execution Envelope は `worker_context_required=true`、`coordinator_may_implement=false`、`serial_fallback_mode=worker_context_only`、remote policy `local_only` を固定する
- Review policy は `requesting-code-review` 第一候補、`hardening_candidate_auto_fix=false`、`candidate_decision_required_before_final_pr=true` とする
- LRG-005 は sibling branch code を merge せず、base `b7a5b989bb856ec0703f490361f7d9898ac521f4` から regression / discoverability artifact を同期する
