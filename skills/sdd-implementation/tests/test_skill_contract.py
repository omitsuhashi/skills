...
class SddImplementationSkillContractTests(unittest.TestCase):
    ...
    def test_all_normal_stage_handoffs_use_a_repository_external_default(self) -> None:
        normalized = " ".join(self.skill_text.split()).lower()
        for stage in (
            "research",
            "spec",
            "plan",
        ...
        self.assertNotIn(".superpowers/research/", self.skill_text)

    def test_repository_local_transient_scratch_is_fail_closed(self) -> None:
        normalized = " ".join(self.skill_text.split()).lower()
        for required in (
    ...
    def test_durable_summary_excludes_raw_handoff_artifacts(self) -> None:
        ...
        for raw_artifact in (
            ...
            self.assertIn(raw_artifact, normalized)
        for summary_field in ("decision", "finding", "repair", "verdict", "evidence identity"):
            self.assertIn(summary_field, normalized)

    def test_git_tree_boundary_requires_repository_validation(self) -> None:
        normalized = " ".join(self.skill_text.split()).lower()
        for prohibited_state in (
            ...
            self.assertIn(prohibited_state, normalized)
        self.assertIn("validate_sdd_transient_artifacts.py", normalized)
        self.assertIn("staged deletion", normalized)
        self.assertIn("historical ancestor", normalized)
