...
owner 以外の actor が作る proposed note 用です。draft は canonical wiki page ではなく、owner review queue として扱います。owner が `promote` / `merge` するまで verified claim として扱わず、`index.md` の現役 page 一覧にも載せません。

draft note は最低限、対象 root、対象 canonical page / claim、提案内容、根拠 source、作成 actor、owner に求める action、作成日を持ちます。新規作成時は `assets/templates/draft-note.md` を雛形として使います。

### Draft Status Convention

draft status は owner review の状態を短く表します。
...
`index.md` は canonical discoverability invariant です。active canonical durable wiki page は `index.md` から発見できなければなりません。draft, rejected note, deferred note, archived duplicate, merged source page は active page として載せません。
