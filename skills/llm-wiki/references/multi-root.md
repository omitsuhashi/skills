...
adapter は、generic field set を解決した後の write decision を次の契約に合わせる。

- proposed note を作れるのは、target root が `Read: allowed` で、`Write: owned` または `propose` で、`Draft Target` が target root 内の root-relative directory として解決済みの場合だけ。
- `Read: restricted` または `Read: no-access` の root には、verified claim も proposed note も書かない。
- `Write: closed` の root には、verified claim も proposed note も書かない。
- target root が不明、`Root URI/Path` が未解決、または `Draft Target` が未設定、未解決、root 外へ解決される場合は、verified claim も proposed note も書かない。
- 上記の stop condition に当たる場合、actor は session user または local governance に確認する。adapter は registry format、approval workflow、root taxonomy を追加で固定せず、この stop decision を返せればよい。
- owner の `draft-review` / `canonicalize` でも、canonical page を直接更新できるのは target root が `Read: allowed`、`Write: owned`、かつ local adapter permission が owner canonical update を許す場合だけ。
...
- `Read` が `restricted` または `no-access` の root には、verified claim も proposed note も書かない。読む必要がある場合は session user または local governance に確認する。
...
- `Draft Target` が未設定、未解決、または root 外へ解決される場合は proposed note を書かず、session user または local governance に確認する。
...
- `promote` は canonical owner が draft-review authority を持ち、target root が `Read: allowed` かつ `Write: owned` で、local contract または adapter が owner canonical update を許す場合に、draft を verified claim として canonical page へ反映する。
- `merge` は canonical owner が draft-review authority を持ち、target root が `Read: allowed` かつ `Write: owned` で、local contract または adapter が owner canonical update を許す場合に、draft の unique な内容を既存 canonical page へ統合する。
- `reject` は採用しない理由を draft 側または `log.md` に残し、現役 queue から外す。
- `defer` は未判断の理由と次に必要な source / owner action を draft 側に残す。
- `promote` / `merge` したら、target root が `Read: allowed` かつ `Write: owned` で owner canonical update が許される場合だけ対象 root の canonical page, `index.md`, `log.md` を更新する。許されない場合は decision と必要な owner action を draft 側または `log.md` に残す。
