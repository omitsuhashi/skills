...
10. `reject` / `defer` でも `log.md` または draft の `Owner Decision` に日付、判断者、理由を残す。履歴なしに draft を削除しない。
...
5. direct update できない場合は `Draft Target` に proposed note を作り、canonical page, `index.md`, `log.md` は直接更新しない。
