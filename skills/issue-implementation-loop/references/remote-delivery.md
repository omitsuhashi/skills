...
Delivery validation also resolves the actual local `epic_base.ref` before any
remote write. The resolved commit must contain the planning base, approved Gate
commit, and every delivery candidate's full `head_sha`; guard-less legacy
packets omit only the unavailable planning-base check. `pr_merged: true` alone
is not integration proof. A missing ref or unreachable required commit blocks
delivery; do not rewrite the branch or mutate the default checkout as recovery.
