...
- **reviewer** owns implementation review for the assigned issue packet and commit range. It reviews requirements, material simplicity, and material risk, then reports only `Critical` / `Important` and residual-risk findings; it does not rewrite scope, perform delivery, or become the worker.
