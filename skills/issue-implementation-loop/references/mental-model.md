...
- **reviewer** owns implementation review for the assigned issue packet and commit range. It reviews requirements, material simplicity, and material risk, then reports only `Critical` / `Important` findings. Separately reported residual risks are limited to material unresolved risks or risks explicitly accepted by a human. It does not rewrite scope, perform delivery, or become the worker.
