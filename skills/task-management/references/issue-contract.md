...
Only `task_project_register` may resume Project registration, and only from an exact prior `task_create` partial receipt that binds the repository, Project URL, marker, Issue identity, recovery state, original requested/default initial-field plan, and exact unfinished transition IDs. A `fields_incomplete` receipt also binds the existing Project item ID. Missing, conflicting, or plan-inconsistent receipt data blocks with zero writes.

The recovery route must never create a second Issue. Multiple Project items linked to the exact Issue and Project are `ambiguous`; do not add a third item or apply defaults. Preserve its current fields except for an explicit field targeted by the requested operation or an exact receipt-declared unfinished transition. A documented unfinished partial-failure step may finish a previously failed default transition on that existing item only when the exact receipt names it. Observe each exact result and preserve every completed Status, Priority, and Due date value. No other existing-item path may apply creation defaults. Do not repeat completed steps.
