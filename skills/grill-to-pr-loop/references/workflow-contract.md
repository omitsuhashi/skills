...
```markdown
| ローカルID | タイトル | レビュー状態 | 実行状態 | ブロック元 | ブロック先 | GitHub Issue | 実装レビュー | PR |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| G2PR-001 | <日本語タイトル> | 承認済み | 実行可能 | なし | G2PR-002 | https://github.com/<org>/<repo>/issues/<n> | 未実施 | 未作成 |
```
...
```markdown
| Wave | ローカルIssue | GitHub Issue | ブランチ | 作業ツリー | Owner/Agent | Write Scope | ベース | 実行状態 | 準備状態 | 検証 | 実装レビュー | PR |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| W1 | G2PR-001 | #123 または ローカルのみ | codex/g2pr-001-slug | <absolute path> | <agent/thread id または 未割当> | <paths/modules> | <sha> | 実行可能 | 準備済み | <commands> | 未実施 | 未作成 |
```
